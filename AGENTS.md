...
with all-RLE or all-repeat sequence metadata before falling back to raw blocks.

Next implementation step: port the remaining superblock metadata paths:
Huffman compressed/treeless literal metadata, FSE compressed sequence table
metadata, then the full `ZSTD_compressSubBlock_multi()` loop with entropy reuse
across sub-blocks.
