...
pub(crate) struct LdmRollingHashState {
    rolling: u64,
    stop_mask: u64,
}

impl LdmRollingHashState {
    pub(crate) fn new(params: LdmParameters) -> Self {
        let max_bits_in_mask = params.min_match_length.min(64);
        let hash_rate_log = params.hash_rate_log;
        debug_assert!(hash_rate_log < 64);

        let stop_mask = if hash_rate_log > 0 && hash_rate_log <= max_bits_in_mask {
            ((1_u64 << hash_rate_log) - 1) << (max_bits_in_mask - hash_rate_log)
        } else {
            (1_u64 << hash_rate_log) - 1
        };
    ...
    pub(crate) fn rolling(&self) -> u64 {
        self.rolling
    }

    pub(crate) fn stop_mask(&self) -> u64 {
        self.stop_mask
    }

    pub(crate) fn reset(&mut self, data: &[u8], min_match_length: usize) {
        debug_assert!(data.len() >= min_match_length);
        let mut hash = self.rolling;
        let mut n = 0;

        while n + 3 < min_match_length {
            hash = gear_step(hash, data[n]);
            hash = gear_step(hash, data[n + 1]);
            hash = gear_step(hash, data[n + 2]);
            hash = gear_step(hash, data[n + 3]);
            n += 4;
        }
        while n < min_match_length {
            hash = gear_step(hash, data[n]);
            n += 1;
        }

        // Matches zstd 1.5.7: `ZSTD_ldm_gear_reset()` does not store `hash`
        // back into `state->rolling`.
        let _ = hash;
    }
    ...
    pub(crate) fn feed(
        ...
        debug_assert!(*num_splits < LDM_BATCH_SIZE);
        let mut hash = self.rolling;
...
#[inline(always)]
fn gear_step(hash: u64, byte: u8) -> u64 {
    hash.wrapping_shl(1)
        .wrapping_add(ZSTD_LDM_GEAR_TAB[byte as usize])
}

const ZSTD_LDM_GEAR_TAB: [u64; 256] = [
    0xf5b8f72c5f77775c,
    0x84935f266b7ac412,
    0xb647ada9ca730ccc,
    0xb065bb4b114fb1de,
