//! Stateful fast block adapter for explicit block-encoding modes.

use alloc::vec::Vec;

use super::{
    append_special_block, encode_prepared_words_block, encode_stored_words_block,
    prepare_block_fast_ext_dict_stored_with_state, prepare_block_fast_ext_dict_words_with_state,
    prepare_block_fast_no_dict_stored_with_state_and_loaded_dict,
    prepare_block_fast_no_dict_words_with_state_and_loaded_dict, target, FastBlockEncodeContext,
    FastBlockEncoding, FastBlockSource, FastExtDictBlockSource, FastMatchState,
};
use crate::encoding::{
...
pub(crate) fn append_block_fast_no_dict_with_state_and_policy_in_mode(
    ...
    if let Some(target_size) = block_encode_mode.target_c_block_size() {
        let prepared = prepare_block_fast_no_dict_words_with_state_and_loaded_dict(
            source.src,
    ...
    let stored = prepare_block_fast_no_dict_stored_with_state_and_loaded_dict(
        source.src,
        source.block_range,
        params,
        repeat_offsets,
...
#[allow(clippy::too_many_arguments)]
pub(crate) fn append_block_fast_ext_dict_with_state_and_policy_in_mode(
    ...
    if let Some(target_size) = block_encode_mode.target_c_block_size() {
        let prepared = prepare_block_fast_ext_dict_words_with_state(
            source,
            params,
            repeat_offsets,
            match_state,
    ...
    let stored =
        prepare_block_fast_ext_dict_stored_with_state(source, params, repeat_offsets, match_state);
