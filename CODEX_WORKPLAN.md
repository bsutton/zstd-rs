...
| `json_logs_32m.jsonl` | 3,392,237 | 826,471 | 1,138,701 | 0.17s | 0.12s | 0.04s |
| `repeated_text_32m.txt` | 31,757 | 2,877 | 3,116 | 0.11s | 0.00s | 0.02s |
| `xorshift_32m.bin` | 33,555,213 | 33,555,213 | 33,555,214 | 0.58s | 0.02s | 0.05s |

Peak RSS from the same run:

| Fixture | Upstream RSS | Current RSS | C zstd -1 RSS |
| --- | ---: | ---: | ---: |
| `decodecorpus_pack.bin` | 6,504 KB | 10,644 KB | 22,068 KB |
| `json_logs_32m.jsonl` | 5,840 KB | 9,584 KB | 19,036 KB |
| `repeated_text_32m.txt` | 5,556 KB | 9,004 KB | 17,804 KB |
| `xorshift_32m.bin` | 6,216 KB | 9,324 KB | 23,580 KB |
...
- The repeat-offset search early exit, sparse long-match indexing, no-match probe step, and text-only wider probing trade about 50 KiB of decodecorpus compression and 2 bytes of repeated-text compression for a large CPU improvement, while JSON is now materially smaller than before the CPU parser shortcuts. The repeat/hash prechecks, hot helper inlining, fixed repeat-candidate loops, candidate-helper inlining, suffix-hash modulo removal, and same-block forward match-length fast path keep output sizes unchanged and improve CPU further. The measured current aggregate CPU is now about 0.34s-0.35s versus about 0.90s before these CPU-focused parser shortcuts.
