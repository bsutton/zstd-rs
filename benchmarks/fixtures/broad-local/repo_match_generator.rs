...
use super::util::likely_lockfile_text;
...
impl MatchGenerator {
    ...
    fn next_sequence(&mut self, mut handle_sequence: impl for<'a> FnMut(Sequence<'a>)) -> bool {
        ...
        loop {
            ...
            for (_repeat_kind, offset) in self.repeat_offset_candidates(literal_len) {
    ...
    fn prefer_dictionary_smaller_offset_same_start(
        ...
        if !matches!(self.file_type_hint, CompressionFileType::DictionaryText)
            ...
            || preferred.match_len + DICTIONARY_SMALLER_OFFSET_MATCH_LOSS_MAX < other.match_len
        {
            return false;
        }
        ...
        other_bits >= preferred_bits + DICTIONARY_SMALLER_OFFSET_BITS_GAIN_MIN
    }
    ...
    fn add_suffix_at(&mut self, idx: usize) {
        ...
        let sidecar_update = if track_second_newest {
            ...
            Some((slot_index, previous_newest))
        } else {
            None
        };
        ...
        if let Some((slot_index, previous_newest)) = sidecar_update {
    ...
    fn add_data(
        ...
        if track_second_newest {
            ...
            if self.use_second_newest_probe && len >= BEST_CURRENT_LONG_HASH_MIN_BLOCK_LEN {
