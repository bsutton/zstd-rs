...
def parse_args():
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=BENCHMARK_TMP / "benchmark-output",
        help="Temporary directory for compressed outputs.",
    )
    parser.add_argument(
        "--current-bin",
        default=str(REPO_ROOT / "target" / "release" / "ruzstd-cli"),
        help="Path to the current ruzstd-cli binary.",
    )
    parser.add_argument(
        "--upstream-bin",
        default=str(BENCHMARK_TMP / "ruzstd-cli-baseline"),
    ...
    parser.add_argument(
        "--csv-output",
        type=Path,
        default=BENCHMARK_TMP / "zstd-rs-benchmark.csv",
    )
    parser.add_argument(
        "--md-output",
        type=Path,
        default=BENCHMARK_TMP / "zstd-rs-benchmark.md",
    )
    parser.add_argument(
        "--commentary",
        default="",
        help="Short note describing what was tried in this benchmark.",
    )
    parser.add_argument(
        "--no-sync",
        action="store_true",
...
def write_markdown(path, rows, csv_path, commentary):
    path.parent.mkdir(parents=True, exist_ok=True)
    by_fixture = {}
    for row in rows:
    ...
    if commentary:
        lines.extend([f"Commentary: {commentary}", ""])
    lines.extend(
        [
...
def main():
    ...
    write_markdown(args.md_output, rows, args.csv_output, args.commentary.strip())
