...
Source CSV: `benchmarks/reports/zstd-bench-compare-level1-config-streamchoice-broad-local.csv`

Commentary: Level-1 experiment: for ConfigText small literal sections, treat the single-stream override as a preferred candidate and fall back to the normal 4-stream path when its estimated literal section is smaller.
