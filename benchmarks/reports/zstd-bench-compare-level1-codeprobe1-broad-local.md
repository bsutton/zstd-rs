...
Commentary: CodeText only, short-line only, <=8 KiB: force dense no-match probing (step 1) to see if repo_main.rs can recover some of C's extra match bytes without touching JSON or binary paths.
...
repo_main.rs                            2,137           2,101      2,105      +1.5%          0.00s         0.00s  0.00s    +0.0%        
