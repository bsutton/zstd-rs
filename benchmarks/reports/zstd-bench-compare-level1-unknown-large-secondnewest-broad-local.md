...
Source CSV: `benchmarks/reports/zstd-bench-compare-level1-unknown-large-secondnewest-broad-local.csv`

Commentary: Extended the retained Fastest current-entry second-newest path from small Unknown blocks to all Unknown non-text blocks, while keeping the no-candidate gate. This revisits the old large-block second-newest idea under the current file-type split so it does not touch .bin guardrails.
...
decodecorpus_z000033                    532,333         571,529    532,333    +0.0%          0.02s         0.00s  0.03s    -50.0%       
